...
class IUserRepository(ABC):
    ...
    def get_deleted_by_email(self, tenant_id: int, email: str) -> User | None:
        """Soft-deleted user with this email (for recreate / restore)."""
    ...
    def restore(
        ...
        *,
        ...
        """Clear deleted_at and refresh profile after a soft-deactivate."""
    ...
    def get_by_id_any(self, tenant_id: int, user_id: int) -> User | None:
        """Active or soft-deleted user."""

    @abstractmethod
    def hard_delete(self, tenant_id: int, user_id: int) -> bool:
        """Permanently remove a soft-deleted user row. Caller must clear FKs first."""
