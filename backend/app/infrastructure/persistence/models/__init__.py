...
class ArchiveScheduleModel(Base):
    """Per-account scheduled incremental archive (not mailbox sync)."""

    __tablename__ = "archive_schedules"
    ...
    folder_id: Mapped[str | None] = mapped_column(String(512), nullable=True)
    folder_path: Mapped[str | None] = mapped_column(String(512), nullable=True)
    limit_per_run: Mapped[int] = mapped_column(Integer, nullable=False, default=500)
    only_with_attachments: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    historical_backfill: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    watermark_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    backfill_watermark_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_run_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    next_run_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    last_job_id: Mapped[int | None] = mapped_column(BigInteger().with_variant(Integer, "sqlite"), nullable=True)
