...
import { getInstallStatus, selfRegister } from "../api/client";
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    getInstallStatus()
      .then((s) => setTenantMode(s.tenant_mode === "multi" ? "multi" : "single"))
      .catch(() => undefined);
  }, []);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const r = await selfRegister({
        name,
        email,
        tenant_slug: tenantMode === "multi" ? tenantSlug : undefined,
      });
      setInfo(r.message);
