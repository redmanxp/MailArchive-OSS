...
function syncDataFormFromSystem(s: SystemSettings) {
  return {
    ...
    tenantMode: (s.tenant_mode === "multi" ? "multi" : "single") as "single" | "multi",
    storageBackend: s.storage_backend === "s3" ? "s3" : "filesystem",
    s3Endpoint: s.s3_endpoint_url || "",
    s3AccessKey: s.s3_access_key || "",
    s3Bucket: s.s3_bucket || "mailarchive",
    s3Region: s.s3_region || "us-east-1",
    s3ForcePath: s.s3_force_path_style !== false,
...
export default function SettingsPage() {
  ...
  const [s3Bucket, setS3Bucket] = useState("mailarchive");
  const [s3Region, setS3Region] = useState("us-east-1");
  const [s3ForcePath, setS3ForcePath] = useState(true);
  ...
  useEffect(() => {
    ...
    getSystemSettings()
      .then((s) => {
        ...
        setTenantMode(synced.tenantMode);
        setStorageBackend(synced.storageBackend);
        setS3Endpoint(synced.s3Endpoint);
        setS3AccessKey(synced.s3AccessKey);
        setS3Bucket(synced.s3Bucket);
        setS3Region(synced.s3Region);
        setS3ForcePath(synced.s3ForcePath);
        setS3Prefix(synced.s3Prefix);
        setS3SecretKey("");
        setMysqlPassword("");
      })
      .catch(() => undefined);
  ...
  async function onSaveData() {
    ...
    try {
      ...
      if (storageBackend === "s3") {
        payload.s3_endpoint_url = s3Endpoint.trim();
        payload.s3_access_key = s3AccessKey.trim();
        payload.s3_bucket = s3Bucket.trim();
        payload.s3_region = s3Region.trim() || "us-east-1";
        payload.s3_force_path_style = s3ForcePath;
        payload.s3_prefix = s3Prefix.trim();
        if (s3SecretKey.trim()) payload.s3_secret_key = s3SecretKey.trim();
      }
      if (dbEngine === "mysql") {
          ...
          <Stack spacing={2} maxWidth={640}>
            ...
            {system ? (
              <>
                <TextField
                  select
                  label={t("settings", "tenantMode")}
                  value={tenantMode}
                  onChange={(e) => setTenantMode(e.target.value as "single" | "multi")}
                  helperText={
                    tenantMode === "single"
                      ? t("settings", "tenantModeSingleHint")
                      : t("settings", "tenantModeMultiHint")
                  }
                  fullWidth
                >
                  <MenuItem value="single">{t("settings", "tenantModeSingle")}</MenuItem>
                  <MenuItem value="multi">{t("settings", "tenantModeMulti")}</MenuItem>
                </TextField>
                <TextField
                  select
                ...
                <TextField
                  ...
                  helperText={t("settings", "dataStorageRootHint")}
                  fullWidth
                />
                <TextField
                  select
                  ...
                  helperText={t("settings", "storageBackendHint")}
                  fullWidth
                >
                  <MenuItem value="filesystem">{t("settings", "storageBackendFs")}</MenuItem>
                  <MenuItem value="s3">{t("settings", "storageBackendS3")}</MenuItem>
                </TextField>
                {storageBackend === "s3" && (
                  <>
                    <TextField
                      ...
                      helperText={t("settings", "s3EndpointHint")}
                      fullWidth
                    />
                    <TextField
                    ...
                    <TextField
                      ...
                      helperText={
                        system.s3_secret_set
                          ? t("settings", "s3SecretKeyHintSet")
                          : t("settings", "s3SecretKeyHint")
                      }
                      fullWidth
                    />
                    <TextField
                      label={t("settings", "s3Prefix")}
                      value={s3Prefix}
                      onChange={(e) => setS3Prefix(e.target.value)}
                      helperText={t("settings", "s3PrefixHint")}
                      fullWidth
                    />
                    <FormControlLabel
                      control={
                        <Switch checked={s3ForcePath} onChange={(e) => setS3ForcePath(e.target.checked)} />
                      }
                      label={t("settings", "s3ForcePath")}
                    />
                  </>
                )}
                <TextField
